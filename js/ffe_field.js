...
var data_dir = "Data/fieldlines/";
var min_length = 50;

function load_next_step(num) {
  if (num > final_frame) return;
  json_loader.load(data_dir + "data" + ("00000" + num).slice(-5) + ".json", function(content) {
    console.log(num);
    var line_group = new THREE.Group();
    var response = JSON.parse(content);
    if (response.length >= min_length) {
      for (var n = 0; n < response.length; n++) {
        var line = make_field_line(response[n]);
        line_group.add(line);
      }
    }
    if (num == init_frame)
...
var vs3, fs3, dataTex, transTex, mat3, cube2, wireframe;
...
var start = function() {
  var dir = "Data/quadru_test3/";
  var txtloader = new THREE.FileLoader();
  txtloader.setResponseType("arraybuffer");
  txtloader.load(dir + "lambda00070.dat", function(content) {
  ...
  wireframe = new THREE.LineSegments(cube_edge, wiremat);
  ...
  function animate() {
    ...
    // mat3.uniforms.eye_pos.value = camera.position;
